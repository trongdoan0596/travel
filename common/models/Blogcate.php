...
class Blogcate extends ActiveRecord {
  ...
  public function attributes(){
        return ['id','title','alias','parent_id','level','fulltxt','path','status','ordering','create_date','last_update','featured','img','icon','metakey','metadesc','lang','user_id','user_modify'];
    }
  public function rules()	{		
         return [
             [['id','title','alias','parent_id','level','fulltxt','path','status','ordering','create_date','last_update','featured','img','icon','metakey','metadesc','lang','user_id','user_modify'],'safe'],
 ...
 public static function getImg($row,$width = 0, $height = 0) {    
  ...
  public static function getPath(&$arr=array(),$parent_id=1){
   ...
   public static function getDetailAlias($alias){
    ...
    public static function getAllBlogcateMobi($cat_id=1,$orderby='title asc'){
        $sql = self::find();
        $sql->where(['status'=>1,'parent_id'=>$cat_id]);
        $sql->andWhere(['>','id',1]);
        $result= $sql->orderBy($orderby)->all();         
