...
/*******frontend***************/
   public static function getMenu($cate_id=0,$lang='en'){
        ...
        $result = Menu::find()
                    ->where('parent_id >0 and status =:status and cate_id =:cate_id  and lang =:lang',
                            array(':status' =>1,':cate_id' =>$cate_id,':lang' =>$lang)
                        )
       ...
       // }
        ...
        switch ($data->type) {
                ...
                case 2://Article Category
                    ...
                    break;
                case 3://Tour
                    /*$infotour = Tour::getDetailTour($data->ext_id);
                     $result   = '#';
                       if(!empty($infotour)){
                            $result = $infotour->createUrl($infotour);
                     }*/
                    //$result= Url::toRoute(array('tour/view','tid'=>$data->ext_id,'title'=>StringHelper::formatUrlKey($data->alias)));
                    $result= Url::toRoute(array('tour/view','language'=>$data->lang,'title'=>StringHelper::formatUrlKey($data->alias)));
                    break;
                 case 4://Tour Category
                    /*$infocate = Tourcate::getDetailTourCate($data->ext_id);
                    $result   = '#';
                    if(!empty($infocate)){
                        $result = $infocate->createUrl($infocate);
                    }*/
