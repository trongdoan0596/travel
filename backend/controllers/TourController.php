...
//upload file
 ...
 public function _createMapImg($model = null, $folder = null,$img = null) { 
           ...
           if (in_array ($ext,$ext_img)) {  
                ...
                $uploadedFile->saveAs($path.$fileName,true); 
              /*  $pathfile = $path.$fileName;                
                $pathfile_thumb = $path.'600_810/'.$fileName;
               Image::getImagine()->open($pathfile)->thumbnail(new Box(600,810))->save($pathfile_thumb ,array('quality' =>100));
               */     
           }
        return $fileName;
}
...
public function loadModel($id) {
      //  $model = Tour::findOne($id);       
        ...
        $model = Tour::findOne(array('id'=>$id,'lang'=>$lang));  
        if ($model === null)
